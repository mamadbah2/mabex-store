...
import { LayoutDashboard, Package, ShoppingCart, LogOut, Eye, Plus } from "lucide-react"
...
const navItems: { label: string; href: string; icon: React.ElementType }[] = [
  { label: "Tableau de bord", href: "/seller", icon: LayoutDashboard },
  { label: "Mes produits", href: "/seller/products", icon: Package },
  { label: "Commandes", href: "/seller/orders", icon: ShoppingCart },
]
    ...
    <aside className="w-72 shrink-0 hidden lg:flex flex-col bg-card border-r border-border h-screen sticky top-0">
      ...
      <div className="p-6 border-b border-border">
        <Link href="/" className="flex items-center gap-3">
          ...
          <span className="font-bold text-xl tracking-tight flex items-center gap-2">
            Mabex
            <Badge className="text-[10px] px-1.5 py-0.5 h-auto">Vendeur</Badge>
          </span>
        </Link>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1">
        ...
        <Link
          ...
          className="flex items-center gap-3 px-4 py-3 rounded-xl transition-colors bg-primary text-primary-foreground hover:bg-primary/90 font-medium"
        >
